...
contains
  ...
  pure function crosspos(x, w) result(res)
    ! take a array x and return the index of cross over between moving average and actual value
    real, intent(in) :: x(:)
    integer, intent(in) :: w
    integer, allocatable :: res(:)
    real :: xavg(size(x))
    logical :: greater(size(x)), smaller(size(x))
    integer :: i
  ...
  pure function crossneg(x, w) result(res)
    ...
    greater = x > xavg
    smaller = x < xavg
    res = pack(res, greater(:size(x)-1) .and. smaller(2:))
